...
6. **コンフリクトの有無を確認する**。PRを作った直後は、ベースブランチが進んでいて後からコンフリクトが発生していることがある。`gh`が使えるなら`gh pr view <branch> --json mergeable,mergeStateStatus`で確認するのが最も確実。`gh`が使えない/判定がつかない場合は、`git fetch origin`した上でベースブランチとのドライランマージ（`git merge --no-commit --no-ff origin/<base>`を試し、コンフリクトが出たら`git merge --abort`で戻す）などローカルで確認する。

   コンフリクトが無ければ何もせず終える。コンフリクトがある場合は、まず自動修正してよいか一言確認する（例:「コンフリクトがあります、自動で直しますか？」）。修正する場合、誰が直すか（このセッション自身が直接編集するか、別のサブエージェントに任せるか）は次の優先順で決める：
   - リポジトリのCLAUDE.mdや、その回の会話で既にやり方が指定されていればそれに従う
   - 指定が無ければ、「自分で直しますか、それとも別のエージェントに任せますか」とユーザーに一言確認してから進める

   自動修正を望まない場合は、コンフリクトがある旨と該当ファイルを伝えるだけに留め、中身の解決はユーザーに委ねる。

7. **mainへの直接merge/pushはこのフロー内で止める**。「作業ブランチをmainへ直接mergeしてpushして」と言われても、このスキルの完了フローとしては応じず、「PR経由にしてください」と案内する。これはこのスキル自身の振る舞いとしてのガードであり、他の`git push`操作全般を監視する汎用フックではない（各リポジトリのCLAUDE.md側の禁止事項と役割分担する）。
...
許可が取れたら削除に入るが、その前に**そのworktreeで動いているdevサーバー等のプロセスが無いか**を一言確認する（例:「このworktreeでdevサーバー等を起動したままになっていませんか？」）。起動時のコマンドはセッションによって様々でこのスキル側からは把握できないため、無理に自動検出しようとせず、ユーザーに直接確認するのが最も確実。動いている場合は停止してよいか確認し、承諾を得てから停止する。

確認を挟まずに`git worktree remove`を実行してしまい、ディレクトリ使用中のエラー等で失敗した場合も、慌てて`--force`に頼らず、同様に「動いているプロセスを止めてもよいか」を確認してから再実行する。

阻害要因が無い（またはユーザーが自分で止めた）ことを確認できたら、次の3つをまとめて削除してよい：
